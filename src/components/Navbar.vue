    ...
    <div class="flex items-center justify-between px-10 py-5 space-x-9">
            ...
            <div class="flex-1 px-8">
                <div class="relative">
                    <input
                        type="text"
                        class="w-full rounded-md outline-none placeholder-header-input-placeholder"
                        style="
                            font-size: 9px;
                            height: 35;
                            padding: 12px 15px;
                    ...
                    <i
                        style=" color: #80B192 "
                        class="fas fa-search absolute text-xs transform -translate-y-1/2 top-1/2 right-5"
                    ></i>
                </div>
            </div>
  ...
  <g id="basket" transform="translate(-712.542 -58.354)">
    ...
    <path id="Vector-4" data-name="Vector" d="M720.667,66.75a.542.542,0,0,0,.542-.542V64.041a.542.542,0,0,0-1.084,0v2.167a.542.542,0,0,0,.542.542Z" fill="#4c4c4c"/>
            ...
            <div v-if="!isLoggedIn && reRender"
                class="flex items-center px-6 space-x-3 text-white rounded-md cursor-pointer"
                style="
            ...
            <div v-if="isLoggedIn && reRender">
                <!-- component -->
<div>
  <div @click="showOptions = !showOptions" class="flex highestzindex justify-center items-center">
      <div @click="open = !open" class="relative border-b-4 border-transparent py-3" :class="{'border-indigo-700 transform transition duration-300 ': open}" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100">
        ...
        <div v-if="showOptions" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-60 px-5 highestzindex py-3 dark:bg-gray-800 bg-white rounded-lg shadow border dark:border-transparent mt-5">
          <ul class="space-y-3 dark:text-white">
            <li class="font-medium">
              <button @click="logOut" href="#" class="flex items-center transform transition-colors duration-200 border-r-4 border-transparent hover:border-indigo-700">
                <div class="mr-3">
    ...
    <section class="SmallScreen bg-white shadow-lg lg:hidden">
        <section>
            <div @click="firstSection = !firstSection" class="flex flex-col gap-5 text-white px-5 bg-bg-primary"
        style=" padding-top: 7.35px; padding-bottom: 7.35px ">
...
>
  ...
  <g id="basket" transform="translate(-712.542 -58.354)">
    ...
    <path id="Vector-4" data-name="Vector" d="M720.667,66.75a.542.542,0,0,0,.542-.542V64.041a.542.542,0,0,0-1.084,0v2.167a.542.542,0,0,0,.542.542Z" fill="#4c4c4c"/>
            ...
            <div class="flex gap-2 align-items-center text-right cursor-pointer py-2 mx-5" @click="category = !category"><span class="ml-auto">Categories</span>
            <svg class="my-auto text-black"
                xmlns="http://www.w3.org/2000/svg"
                width="9.096"
                height="6.548"
                viewBox="0 0 7.096 3.548"
...
this.reRender = false;
    ...
    logOut(){
 axios.delete(process.env.VUE_APP_API_URL + 'customer/logout',{withCredentials:true})
        .then((res) => {
          console.log(res)
          this.$router.go(0)
      }).catch((error) => {
